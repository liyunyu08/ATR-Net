...
from models.ATR_Net import ATR_Net
from task_token_test import test_main, evaluate
import logging
...
def train_main(args):
    ...
    train_loader = DataLoader(dataset=trainset, batch_sampler=train_sampler, num_workers=6, pin_memory=True)
    ...
    val_loader = DataLoader(dataset=valset, batch_sampler=val_sampler, num_workers=6, pin_memory=True)
    ''' fix val set for all epochs '''
    val_loader = [x for x in val_loader]

    model = ATR_Net(args,resnet=args.resnet).cuda()
